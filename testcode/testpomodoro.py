...
from PSscript import CHANGEVOLUME30_FILE, CHANGEVOLUME100_FILE
from data import USER_RESPOND_DATA_FILE, TIMEDATA_FILE
...
class PopupWindow(tk.Tk):
    ...
    def on_closing(self):
        user_respond = open(str(USER_RESPOND_DATA_FILE), 'w')
    ...
    def restore_volume():
        ...
        p = subprocess.run(['C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe', '-ExecutionPolicy',
                            'Unrestricted', '-WindowStyle', 'Hidden',
                            str(CHANGEVOLUME100_FILE)])


class StartWindow(tk.Tk):
    def __init__(self):
        super().__init__()
        self.taskbar_icon = tk.PhotoImage(file=str(IMAGEFILE))
    ...
    def on_click(self):
        ...
        if message_data == '':
            message_data = 'None'
        data_file = open(str(TIMEDATA_FILE), 'w')
    ...
    def on_closing(self):
        user_respond = open(str(TIMEDATA_FILE), 'w')
        user_respond.write('state close')
        user_respond.close()
        self.destroy()
...
def notify():
    ...
    data = get_data_in(str(TIMEDATA_FILE))
    notify_in_sec = 30
...
def main():
    ...
    while state_on:
        a = StartWindow()
        a.mainloop()
        message = get_data_in(str(TIMEDATA_FILE))
        if message['state'] == 'close':
            break
        notify()
        message = get_data_in(str(USER_RESPOND_DATA_FILE))
        if message['state'] == 'close':
            break
